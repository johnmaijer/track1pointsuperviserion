...
parser.add_argument("--logEnpoch", type=int, default=10, help="print the train loss for each logEpoch")
...
def train():
    ...
    for idx_epoch in range(epoch_state, opt.nEpochs):
        ...
        if (idx_epoch + 1) % opt.logEnpoch  == 0 or (idx_epoch + 1) == opt.nEpochs:
        ...
        if total_loss_list[-1] < opt.LESPS_Tloss and start_click == 0 or (idx_epoch + 1) % opt.test_iter == 0 or idx_epoch == opt.nEpochs:
            ...
            if total_loss_list[-1] < opt.LESPS_Tloss:
                print('update start')
                start_click = 1
        ...
        # subsequent update
        if start_click == 1 and (idx_epoch + 1) % opt.LESPS_f == 0 or (idx_epoch + 1) % opt.test_iter == 0 or idx_epoch == opt.nEpochs:
            save_pth = opt.save + '/' + opt.dataset_name + '/' + opt.save_perdix + '_' + str(idx_epoch + 1) + '.pth.tar'
            save_checkpoint({
                'epoch': idx_epoch + 1,
                'state_dict': net.state_dict(),
